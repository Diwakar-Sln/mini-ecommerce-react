...
const Home = () => {
  let {products, isLoading, error} = useFetchProducts(productApi.getProducts);
  return (
    <div>
        ...
        isLoading ? <div>Loading...</div> :
          products.length === 0 ? <div>{error}</div> :
            <div style={{ display: "grid", alignItems: "center", gridTemplateColumns:"auto auto auto auto auto", gap: "20px" }}>
              {
                products.map((product) => <Card key={product.id} id={product.id} title={product.title} image={product.thumbnail} price={product.price} />)
              }
            </div>
      }
    </div>
